...
# 2日たっても終わらない
# GPUが使えないから？
# ある程度で終わる範囲でパラメータを選別してチューニングする必要がある


def create_model(n_layers, activation, mid_unit, dropout_rate, classes, input_shape):
    model = Sequential()
    model.add(Dense(mid_unit, activation=activation, input_shape=(input_shape,)))
...
def objective(trial):
    ...
    # 学習モデルの構築と学習の開始
    model = create_model(n_layers, activation, mid_units, dropout_rate, y_train_data.nunique(), x_train.shape[1])
...
if __name__ == '__main__':
    ...
    basicConfig(filename=RESULT_DIR + file_prefix + '_parameter_tuning_mlp.py.log', level=INFO, format=log_format)
